...
def test_get_menu_not_auth(restaurant) -> None:
    menu = Menu.objects.create(title='test_menu',
                               file=SimpleUploadedFile('temp_file.pdf',
                                                       b'some test content'),
...
@pytest.mark.django_db
def test_get_menu_auth_as_admin(restaurant: object,
                                user_payload_as_admin: dict) -> None:
    menu = Menu.objects.create(title='test_menu',
                               file=SimpleUploadedFile('temp_file.pdf',
                                                       b'some test content'),
                               restaurant=restaurant)
...
@pytest.mark.django_db
def test_create_menu_authenticated_but_not_admin(user_payload_not_admin: dict,
                                                 restaurant: object) -> None:
    payload = dict(title='test_title',
                   file='temp_file.pdf',
...
@pytest.mark.django_db
def test_create_menu_authenticated_as_admin_with_no_data(
        user_payload_as_admin: dict) -> None:
    User.objects.create_user(**user_payload_as_admin)
    user_data = client.post(reverse('login'),
                            user_payload_as_admin,
...
@pytest.mark.django_db
def test_create_menu_authenticated_as_admin_with_data(
        user_payload_as_admin: dict, restaurant: object) -> None:
    temp_file = tempfile.TemporaryFile(suffix='.pdf',
                                       prefix='temp_',
                                       dir=os.path.join(settings.BASE_DIR /
                                                        'media/menus'))
    temp_file.write(b'some content')
    temp_file.seek(0)
    payload = dict(title='test_menu', file=temp_file, restaurant=restaurant.id)
    User.objects.create_user(**user_payload_as_admin)
    user_data = client.post(reverse('login'),
                            user_payload_as_admin,
                            format='json')
...
def test_list_menu_not_auth(restaurant: object) -> None:
    assert len(Menu.objects.all()) == 0
    Menu.objects.create(title='test_menu',
                        file=SimpleUploadedFile('temp_file.pdf',
                                                b'some test content'),
                        restaurant=restaurant)
    assert len(Menu.objects.all()) == 1
...
@pytest.mark.django_db
def test_list_menu_auth_as_admin(user_payload_as_admin: dict,
                                 restaurant: object) -> None:
    assert len(Menu.objects.all()) == 0
    Menu.objects.create(title='test_menu',
                        file=SimpleUploadedFile('temp_file.pdf',
...
@pytest.mark.django_db
def test_list_menu_auth_not_admin(restaurant: object,
...
@pytest.mark.django_db
def test_update_menu_auth_as_admin(restaurant: object,
                                   user_payload_as_admin: dict) -> None:
    assert len(Menu.objects.all()) == 0
    Menu.objects.create(title='test_menu',
                        file=SimpleUploadedFile('temp_file.pdf',
    ...
    res = client.patch(reverse('update_menu', kwargs={'pk': 1}),
                       new_payload,
                       format='multipart')
...
@pytest.mark.django_db
def test_update_menu_auth_as_admin_no_data(
        restaurant: object, user_payload_as_admin: dict) -> None:
    assert len(Menu.objects.all()) == 0
    menu = Menu.objects.create(title='test_menu',
                               file=SimpleUploadedFile('temp_file.pdf',
                                                       b'some test content'),
...
@pytest.mark.django_db
def test_update_menu_auth_not_admin(restaurant: object,
                                    user_payload_not_admin: dict) -> None:
    assert len(Menu.objects.all()) == 0
    Menu.objects.create(title='test_menu',
                        file=SimpleUploadedFile('temp_file.pdf',
...
@pytest.mark.django_db
def test_delete_menu_auth_as_admin(restaurant: object,
                                   user_payload_as_admin: dict) -> None:
    assert len(Menu.objects.all()) == 0
    Menu.objects.create(title='test_menu',
                        file=SimpleUploadedFile('temp_file.pdf',
...
@pytest.mark.django_db
def test_delete_menu_auth_not_admin(restaurant: object,
                                    user_payload_not_admin: dict) -> None:
    assert len(Menu.objects.all()) == 0
    Menu.objects.create(title='test_menu',
                        file=SimpleUploadedFile('temp_file.pdf',
